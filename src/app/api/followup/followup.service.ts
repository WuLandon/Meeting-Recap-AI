...
export async function generateFollowUpEmail(meeting: MeetingOutput): Promise<string> {
  ...
  • Keep introductory paragraphs concise and well-paced for readability. Split or combine them naturally based on context. The intro should provide a high-level overview of the meeting without repeating details already covered in decisions or action items.
  • End with a short, natural closing line that ties back to the project or next steps, not a generic motivational phrase — unless such a phrase naturally fits the meeting’s tone.
